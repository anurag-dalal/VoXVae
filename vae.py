...
class VAE(nn.Module):
    ...
    def loss(self, inputs, outputs, mu, sigma):
        # Reconstruction loss
        ...
        KLD /= inputs.size(0) * inputs.size(2) * inputs.size(3) * inputs.size(4)  # Normalize by batch and dimensions
        # fake positive handling
        gt = inputs < 0.5
        pred = torch.sigmoid(outputs) > 0.5
        fp = torch.bitwise_and(gt,pred).sum().float()
        # print('fp', fp, fp.dtype)
        fp /= inputs.size(0) * inputs.size(2) * inputs.size(3) * inputs.size(4)
        return BCE, KLD, fp
...
# model = VAE().to(device)
# x = torch.rand(2, 1, 32, 32, 32).to(device)
# y = model(x)
